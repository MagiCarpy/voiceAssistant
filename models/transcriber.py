...
class Transcriber:
    ...
    "automatic-speech-recognition", model=default_transcriber_model, device=-1
        ...
        preprocessing = Process(target=self.transcriber_worker, args=(self.frames_q, self.transcriptions_q, self.event))
        preprocessing.start()
    ...
    def transcriber_worker(self, frames_queue, transcription_queue, event):
        while True:
